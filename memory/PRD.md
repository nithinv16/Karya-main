...
## Changelog
- 2026-02 (f): P1 signed short-lived URLs (HMAC-SHA256 + expiry) on `/api/files/{path}` so Twilio can attach report photos to WhatsApp messages; timing-safe compare. P2 extracted `/reports/*` block (~250 lines) into `/app/backend/routes/reports.py` behind a `Deps` factory (server.py down to ~1227 lines). New "Send now via WhatsApp" one-tap button on each report card → `POST /api/reports/{id}/whatsapp/quick` (uses project client phone + user default). Verified 12/12 (iteration_13.json).
